...
- **`request_class_classification.ipynb`:** Evaluates two distinct clustering methodologies to address the operational complexity of the current request system. The primary goal is to consolidate approximately 400 highly granular request classes into a more manageable set of 100 cohesive groups. By leveraging semantic similarity within the REQUEST_CLASS and DESCRIPTION fields, the model aims to streamline reporting and improve operational efficiency by reducing the number of classes to 100 clusters. A K-Means approach and an Agglomerative approach were both used to evaluate clustering, with the Agglomerative model preferred over K-Means because it is deterministic, captures the natural hierarchy of maintenance tasks, and allows the "optimal" number of groups to be determined via a dendrogram rather than being predefined.
